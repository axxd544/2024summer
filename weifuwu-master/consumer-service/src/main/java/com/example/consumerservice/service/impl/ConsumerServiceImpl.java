...
import com.example.consumerservice.controller.MinioUploadController;
...
import static com.example.consumerservice.constant.Constants.SALT;
...
public class ConsumerServiceImpl extends ServiceImpl<ConsumerMapper, Consumer>
    ...
    @Override
    public R getConsumerByUsername(String username) {
        //此方法用于返回歌曲创建者的信息（如果有)
        QueryWrapper<Consumer> consumerQueryWrapper = new QueryWrapper<>();
        consumerQueryWrapper.eq("username", username);
        return R.success(null, consumerMapper.selectList(consumerQueryWrapper));
    }
